...
| Tier selection at creation | **Partial** | N-80, 18 Sept. The wizard's step 1 offers Light/Medium/Heavy — a storage tier, per D-60 coexisting with rather than replacing the duration ladder. Unset by default, so an operator who skips it gets exactly today's behaviour. **Missing:** a duration-plan step (Deliver/Keep/Cinema); a Deliver catalogue still needs the 90-day term from N-24. |
...
| Add storage | **Partial** | N-79, 18 Sept. No purchase flow or proration still — but a wedding at its cap now has an **Ask for more space** button, which reaches the platform, who can grant a catalogue-scoped override in minutes. |
...
| Included term matches what is sold | **Built** | Each plan's own length, from its first Publish (N-28, then N-120): Deliver 90 days, Keep and Cinema twelve months. |
...
| "Couple login" becomes "client login" | **Built** — D-42 | N-78, 18 Sept. Outward copy only: the door, the platform list, the register page ("partner account" → "studio account"), the guide. "The couple" stays where it means the couple. Checked and unchanged: every notification template, since none named "couple" outside a `{coupleName}` placeholder. |
| Buy add-on space | **Partial** | The price exists (`PRICING-MODEL.md` §3); buying it is still N-20's checkout. **Interim done, N-79, 18 Sept:** *Ask for more space*, the credit request's shape, now targeting the catalogue's own grant (D-60) rather than the org's. |
| Tiered plans by storage | **Built** — D-60 | Light 5 · Medium 50 · Heavy 100 GB, coexisting with the duration-priced ladder in `PRICING.md` §1 rather than replacing it (D-60 settled what D-44 left open). Chosen in the wizard, written as a catalogue-scoped entitlement. Custom (100–300 GB) is a "contact us" note, not a self-service tier — it has no fixed number. Prices for the three tiers are still unset. N-80, 18 Sept. |
...
| Prices are platform settings | **Built** — D-61 | N-118, 19 Sept. `plans` is the price list: `/admin/platform/pricing` edits any price (empty = not for sale, which is not free), the suggested-retail ranges and the Studio plan's credit bundle; every change audited with from/to and a reason; the marketing page, the credits card, the extra-storage line and the refusal panel all read it, and a test fails if a figure is typed back into code or a help page. Live on staging and production, 19 Sept. **Not yet true:** a purchase does not record what was charged — there is no purchase until N-20 — and term lengths are still copy (N-120). |
| Studio credits are typed | **Built** — D-61 | N-119, 19 Sept. A credit is of a plan — Deliver, Keep or Cinema — and a wedding is on one, chosen in the wizard beside the storage tier and changeable on its overview **until it is first published**, after which it is fixed (publishing spent a credit of *that* plan; `publishedAt` survives an unpublish, so "ever published" is what is asked). Publishing spends the soonest-expiring credit **of the wedding's plan**; an empty basket refuses by name and, when another basket holds credits, says so and how to use it. The dashboard, the Studio page, the platform console's grant form (a plan picker, on the audit row) and the credit request (deduped per plan per day) are all typed; existing weddings and credits are `deliver`, stated in migration `0030`. A couple holds no basket, so the wizard shows them the plan without a count. |
| Studio starter grant: 2 Deliver + 1 Cinema | **Partial** — D-61 | The bundle exists and is editable on the pricing page (N-118, 19 Sept) and the marketing page describes it from there. Nothing *grants* it yet: registration still gives one credit (N-65), now a Deliver one since N-119 until N-113 reads the bundle. |
| Term starts at first Publish | **Built** — D-61 | N-120, 19 Sept. A wedding has **no term** until its first Publish (`includedUntil` is null: *not started*, never *expired*); Publish then sets it from the wedding's plan — Deliver 90 days, Keep and Cinema 12 calendar months, held as `plans.term_days` / `term_months` (at most one), so the length is a setting and the marketing table reads it. A republish never restarts it, and Publish never moves an end date the platform set on purpose. A plan with no term refuses the first Publish *before* the credit is spent. Existing published weddings kept their dates; never-published drafts were emptied by migration 0031. **Not yet:** the console cannot edit a term — it changes by migration, until someone needs it to. |
| Coupon codes — discounts and rewards | **Built** — D-61 | N-121, 19 Sept. `/admin/platform/coupons` creates a coupon (percent off, an amount off, or a **reward** of typed credits — studios only, no payment), with a required campaign label, a window in India's timezone, an overall limit and one per payer (default one), and a door and product scope; switches one off or on; and reads a cohort back by campaign. A coupon is **never edited or deleted** — a redeemed code is history — and every change is audited. A studio redeems a reward in *Your studio → Credits → Have a code?*, which grants the credits and records the redemption in one atomic step under a row lock, so the last use of a code cannot be taken twice. Every way a code can fail — mistyped, unknown, disabled, expired, used up, wrong door, wrong product, wrong kind — gives one answer, and lookups are rate-limited per studio and per address. `applyCoupon(code, product, payer)` is the server-side quote N-20's checkout will charge from (the browser never sends a price or a discount; one code per call; a percentage rounds down); it reads only, and `Repository.redeemCoupon` is what N-20 calls once there is a payment to attach. **Not yet:** nothing calls `applyCoupon` until N-20, so a discount code cannot be used by anyone yet. |
