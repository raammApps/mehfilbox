...
`useHlsPlayback.ts`'s `isManifest` check means a `.webm` never takes the `hls.js`/`MediaSource`/
`blob:` branch at all — the exact path a broken CSP directive broke in production behind an
all-green suite. `getPlaybackToken` now returns a real HLS manifest,
`/media/hls/playlist.m3u8`, generated by `scripts/make-sample-hls.mjs` from Playwright's own
Chromium: `MediaRecorder` with `mimeType: 'video/mp4;codecs=avc1.42E01E'` produces genuine
ISO-BMFF (`ftyp`+`moov` once, then `moof`+`mdat` fragments), no ffmpeg and no stock footage
(doc 12 §1, doc 13 §8), same rule `make-sample-video.mjs` already followed for the `.webm`.

Two real bugs found only by inspecting the actual generated bytes, not by reasoning about the
generation code:

1. A first version assumed `ftyp` and `moov` always arrive in the same `ondataavailable`
   callback; Chrome's muxer does not promise that, and the first real run shipped a 36-byte
   `ftyp`-only init segment that could never initialise a `MediaSource`. Fixed by concatenating
   every chunk into one byte stream before walking ISO-BMFF boxes, rather than trusting chunk
   boundaries to mean anything.
2. `requestData()` does not flush a fragment on demand for this codec — confirmed with a
   diagnostic that called it every second for 18s and got an empty chunk back every time except
   the first (a bare `ftyp`) and one enormous chunk at `stop()`. A version that paired fragments
   with the wall-clock gaps between `requestData()` calls produced a manifest whose declared
   total duration (~8s) was under half of what was actually encoded (~18s): the fragments existed
   on disk, the manifest just never pointed at enough of them. `e2e/playback.spec.ts`'s "Start
   over" test — which seeks to 8s — is what surfaced it, by tripping the 0.95 completion
   threshold on a video the manifest claimed was only 8s long. Fixed by reading each fragment's
   real duration out of its own `moof/traf/trun` box (`trun`'s per-sample durations, confirmed
   present via its flags) against the track's timescale (`moov/mdia/mdhd`) — the script no longer
   guesses timing from when it happened to call `requestData()`.

Verified three ways before calling it done: independent Python box-walking of every generated
file (not the same JS logic that produced them); the full `e2e/playback.spec.ts` suite, which now
genuinely exercises `hls.js`/`MediaSource`/`blob:` instead of silently bypassing them; and a live
`pnpm dev` session with `VIDEO_DRIVER=fake`, confirming `video.src` is a real `blob:` URL,
`readyState` reaches 4, and `currentTime` advances during `.play()`, with zero console or network
errors. Full suite: 156 passed / 56 skipped E2E, 691 unit/component, unchanged from before this
ticket.
