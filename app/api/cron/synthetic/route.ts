...
export async function GET(request: Request) {
  return route('cron/synthetic', async () => {
    ...
    if (verdict.kind !== 'ok') {
      ...
      return respond(fail('resolve', `catalogue "${slug}" resolved as ${verdict.kind}`))
    }
    ...
    if (!playable) {
      return respond(fail('film', `no ready film with a provider id in "${slug}"`))
    }
    ...
    try {
      ...
      if (!ticket.playbackUrl) return respond(fail('playback', 'no playback URL was issued'))
      steps.push({ step: 'playback', ok: true })
    } catch (error) {
      return respond(fail('playback', (error as Error).message))
    }
